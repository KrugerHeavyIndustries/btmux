...
void mech_pickup(dbref player, void *data, char *buffer)
{
    ...
    /* Drop anything the target is carrying */
    ...
    SendDebug("#%d has picked up #%d", mech->mynum, target->mynum);
}
...
void mech_attachcables(dbref player, void *data, char *buffer)
{
    ...
    MechTowedBy(target) = towMech->mynum;
    MechSwarmTarget(target) = -1;
...
void mech_dropoff(dbref player, void *data, char *buffer)
{
    ...
    correct_speed(mech);
    SendDebug("#%d has dropped off #%d", mech->mynum, target->mynum);
}
